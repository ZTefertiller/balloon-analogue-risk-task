...
df <- dataset %>% mutate(color_max = case_when(
  balloon_color == 'y' ~ 64,
  balloon_color == 'o' ~ 96,
  balloon_color == 'b' ~ 128
))
...
fit <- stan(
  file = "/Users/zachtefertiller/Desktop/STL_toy/STL_HBM.stan",  # path to your Stan model file
  data = stan_data,                   # your prepared data list
...
mod <- cmdstan_model("/Users/zachtefertiller/Desktop/STL_toy/STL_HBM.stan")

fit <- mod$sample(
  data = stan_data,
  iter_sampling = 2000,
  chains = 4,
  parallel_chains = 8, # number of chains to run in parallel
