...
function storePoints() {
    ...
    if (inflations > 0) {
        ...
        message.textContent = 'Sie müssen den Ballon mindestens einmal aufblasen.';
        message.style.display = 'block';
        setTimeout(() => {
...
function popBalloon() {
    ...
    popAudio.play(); 
    message.style.display = 'block';
    message.innerHTML = `Der Ballon ist geplatzt! Sie haben kein Geld verdient.<br>Nächster Durchgang in 5 Sekunden (oder drücken Sie Leertaste, um fortzufahren).`;
    let trialData = {
        participant_id: participantID,
        trial_number: index,
        balloon_color: currentBalloonColor,
...
function endTask() {
    ...
    document.getElementById('totalPoints').style.display = 'none';
    message.style.display = 'block';
    message.textContent = `Sie haben €${totalMoney.toFixed(2)} verdient. Gut gemacht!`;
